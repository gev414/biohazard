...
public final class InfectionMedicineItem extends PotionItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (kind == Kind.FULL_CURE && !player.hasEffect(HordesInfection.INFECTED)) {
            if (!level.isClientSide) {
                sendStatus(player, "message.rotwire.infection.not_infected");
            }
    ...
    private static void cure(LivingEntity entity) {
        if (!entity.hasEffect(HordesInfection.INFECTED)) {
            sendStatus(entity, "message.rotwire.infection.not_infected");
            return;
        }
        ...
        if (entity.removeEffect(HordesInfection.INFECTED)) {
            ...
            sendStatus(entity, "message.rotwire.infection.cured");
        }
    }
    ...
    private static void suppress(LivingEntity entity) {
        ...
        if (current != null) {
            ...
            sendStatus(entity, "message.rotwire.infection.suppressed");
        } else {
            sendStatus(entity, "message.rotwire.infection.immunity");
        }
    ...
    public void appendHoverText(
        ...
        tooltip.add(Component.translatable("tooltip.rotwire." + suffix).withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable("tooltip.rotwire." + suffix + ".detail")
